...
const D = {
  ...
  chat: "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.557 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z",
      ...
      {[1, 2, 3, 4, 5].map((star) => {
        ...
        return (
          <svg key={star} viewBox="0 0 24 24"
            ...
            stroke={filled ? "#EF9F27" : "rgba(255,255,255,0.2)"}
            strokeWidth="1.5"
...
export default function Profile() {
  ...
  // ── Fetch user if viewing another profile ──
  useEffect(() => {
    if (isOtherProfile) {
      setLoading(true);
      getUserById(userId)
    ...
    <>
      ...
      {showNotification && (
        <div style={{ position: "fixed", top: 20, right: 20, zIndex: 999, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", borderRadius: 14, padding: "14px 18px", maxWidth: 320, display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          <button onClick={() => setShowNotification(false)} style={{ background: "none", border: "none", color: "rgba(251,191,36,0.5)", cursor: "pointer", fontSize: 16 }}>✕</button>
        </div>
      )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 40, paddingLeft: 8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>
                {isTransporteur ? "Transporteur Portal" : "Client Portal"}
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 24, alignItems: "start" }}>
                ...
                <div style={{ ...glass, padding: "28px 24px", textAlign: "center" }}>
                  <div style={{ position: "relative", width: 80, height: 80, borderRadius: "50%", background: `linear-gradient(135deg,${accent.from},${accent.to})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, fontWeight: 800, margin: "0 auto 16px", boxShadow: `0 8px 24px ${accent.glow}`, overflow: "hidden" }}>
                    {previewUrl ? (
                    ...
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <button 
                        ...
                        style={{ 
                          ...
                          marginBottom: profilePictureFile ? 8 : 0
                        }}
                      >
                        📷 Changer la photo
                      </button>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                          <button 
                            ...
                            style={{ 
                              ...
                              padding: "8px 12px", 
                              borderRadius: 8, 
                              background: uploading ? "rgba(34,197,94,0.5)" : "linear-gradient(135deg,#22c55e,#15803d)",
                              border: "none",
                              color: "#fff",
                              fontSize: 11,
                              fontWeight: 600,
                  ...
                  {/* Note moyenne dans l'avatar card */}
                ...
                <div style={{ ...glass, padding: "28px 32px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    {infoFields.map(item => (
                      <div key={item.label} style={{ background: item.editable && isEditing ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.02)", border: item.editable && isEditing ? `1px solid ${accent.activeBorder}` : "1px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: "14px 16px" }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}>
                          {item.label}
                        ...
                        {item.editable && isEditing ? (
                          item.type === "select" ? (
                            ...
                            <input type="text" name={item.field} value={formData[item.field]}
                              onChange={handleInputChange} placeholder={item.ph} style={inp} />
                          )
                        ) : (
                          <div style={{ fontSize: 13, fontWeight: 600, color: item.value ? "#e2e8f0" : "rgba(255,255,255,0.4)" }}>
              ...
              {/* Row 2 — Section Rating */}
              {reviewData && (
                <div style={{ ...glass, padding: "28px 32px" }}>
                  <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 24px", paddingBottom: 16, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                    Évaluations reçues
                  </h2>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 32 }}>
                          ...
                          {starDist.map(({ star, count }) => {
                            ...
                            return (
                              <div key={star} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", width: 8, textAlign: "right" }}>{star}</span>
                                ...
                                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", width: 16 }}>{count}</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 360, overflowY: "auto" }}>
                        {reviewData.reviews.map((review) => (
                          <div key={review._id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "14px 16px" }}>
                            ...
                            {review.comment && (
                              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", marginTop: 8, fontStyle: "italic", padding: "8px 10px", background: "rgba(255,255,255,0.05)", borderRadius: 8 }}>
                                "{review.comment}"
                              </div>
                            )}
